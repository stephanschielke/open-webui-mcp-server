# Production Publishing Workflow - Complete Summary
...
- `mise run release:check` - Run pre-release checks (tests, linter, build)
- `mise run release:tag X.Y.Z` - Create and push a release tag
- `mise run release:prepare X.Y.Z` - Full release preparation (checks + tag)
...
2. `mise.toml`
    - Added `release:check` task
    - Added `release:tag` task
    - Added `release:prepare` task
...
**The production publishing workflow is now complete with OIDC authentication, comprehensive guardrails, and full documentation.**
